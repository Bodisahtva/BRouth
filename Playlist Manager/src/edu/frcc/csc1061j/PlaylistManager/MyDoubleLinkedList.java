...
public class MyDoubleLinkedList<E> implements List<E> {
	...
	public int count(MyDoubleLinkedList <Song> playlist)
	{
		int count = 0;
		...
		return count;		//returns number of songs in the list
	}
	...
	public void reverseList ()
	{
		for (Node node = head; node != null; node = node.next)
		{
			Node temp = node.next;		//creates temporary location to move objects to 
			node.next = node.prev;		//moves next into previous location
			node.prev = temp;			//moves previous to temporary
		}
		Node temp = head;
		head = tail;
		tail = temp;
